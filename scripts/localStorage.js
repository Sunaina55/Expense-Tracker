export default class display {
    ...
    static saveTrans(trans = {}) {
        ...
        const formattedTrans = {
            id: trans.id || Date.now(),
            amount: Number(trans.amount),
            category: trans.category || trans.tag || "General",
            note: trans.note || "",
            date: trans.date || new Date().toISOString()
        };

        const existingIndex = allTrans.findIndex(
            item => item.id === formattedTrans.id
        );

        if (existingIndex !== -1) {
            allTrans[existingIndex] = formattedTrans;
        } else {
            allTrans.unshift(formattedTrans);
        }
