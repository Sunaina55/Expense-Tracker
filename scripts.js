...
function getCatTotals(list) {
  const t = {};
  list.forEach(e => { t[e.cat] = (t[e.cat] || 0) + e.amt; });
  return t;
}

// ── Populate Selects ───────────────────────────────────────
function populateSelects() {
  const opts = CATEGORIES.map(c => `<option value="${c.name}">${c.emoji} ${c.name}</option>`).join('');
...
// ── Tabs ───────────────────────────────────────────────────
const tabTitles = { dashboard: 'Dashboard', add: 'Add Expense', budgets: 'Budgets', recurring: 'Recurring' };
...
function renderMetrics() {
  ...
  balEl.className = 'metric-value ' + (income ? (bal < 0 ? 'red' : 'green') : '');
...
function renderExpenseList() {
  const me = getMonthExpenses();
  let list = activeFilter === 'All' ? me : me.filter(e => e.cat === activeFilter);
  ...
  if (!list.length) {
    el.innerHTML = '<div class="empty-state">No transactions for this filter.</div>';
    return;
  }
...
document.getElementById('addBtn').addEventListener('click', () => {
  ...
  if (!desc)       { info.textContent = 'Please enter a description.'; return; }
  if (!amt || amt <= 0) { info.textContent = 'Please enter a valid amount.'; return; }
  if (!date)       { info.textContent = 'Please select a date.'; return; }
...
function renderBudgets() {
  ...
  if (!keys.length) { el.innerHTML = '<div class="empty-state">No budgets set yet.</div>'; return; }
...
function renderRecurring() {
  ...
  if (!recur.length) { el.innerHTML = '<div class="empty-state">No recurring expenses. Mark one when adding.</div>'; return; }

  const total = recur.reduce((s, e) => s + e.amt, 0);
  el.innerHTML = `<div class="recur-summary">Total monthly recurring: <strong>${fmt(total)}</strong></div>` +
    recur.map(e => {
      const cat = CAT_MAP[e.cat] || CAT_MAP['Other'];
      return `
        <div class="expense-item">
...
function renderCharts() {
  ...
  if (cats.length) {
    ...
    pieCtx.canvas.closest('.chart-card').querySelector('.chart-wrap').innerHTML =
  ...
  me.forEach(e => { daily[new Date(e.date).getDate() - 1] += e.amt; });
  ...
  barInst = new Chart(document.getElementById('barChart').getContext('2d'), {
    ...
    options: {
      ...
      scales: {
        x: {
          grid: { display: false },
          ticks: { font: { size: 12, family: 'DM Sans', weight: 500 }, color: '#6B7280', maxTicksLimit: 12 }
        },
        y: {
...
// ── Sort change ────────────────────────────────────────────
document.getElementById('sortTrans').addEventListener('change', renderExpenseList);
